...
pub fn msgSend(comptime ReturnType: type, target: anytype, selector: SEL, args: anytype) ReturnType {
    ...
    const FnType = blk: {
        ...
        }
        {
            ...
            break :blk switch (args_meta.len) {
                ...
                1 => *const fn (@TypeOf(target), SEL, args_meta[0].type) callconv(.C) ReturnType,
                2 => *const fn (@TypeOf(target), SEL, args_meta[0].type, args_meta[1].type) callconv(.C) ReturnType,
                3 => *const fn (@TypeOf(target), SEL, args_meta[0].type, args_meta[1].type, args_meta[2].type) callconv(.C) ReturnType,
                4 => *const fn (@TypeOf(target), SEL, args_meta[0].type, args_meta[1].type, args_meta[2].type, args_meta[3].type) callconv(.C) ReturnType,
    ...
    var func: FnType = @ptrCast(&c.objc_msgSend);
